...
hs <- makeParamSet( 
  makeIntegerParam("min_data_in_leaf", lower=    1500L   , upper=  8000L), #Dejar ir. Puede ser 1k
  makeIntegerParam("num_leaves",       lower=   7L   , upper=  4095L),
  makeIntegerParam("max_depth",       lower=   2L   , upper=  63L),
  makeNumericParam("bagging_fraction", lower=   0.4   , upper=  1.0),
  makeNumericParam("feature_fraction", lower=   0.4   , upper=  1.0),
  makeNumericParam("learning_rate", lower=   0.004   , upper=  0.005), #Esto es una chanchada, lo hice así porque me dieron error las maneras en las que intenté dejar fijo el LR
...
PARAM$experimento  <- "HT7231_FE_paramdam1_lr005"

PARAM$input$dataset       <- "./datasets/dataset_7110.csv.gz"
...
PARAM$hyperparametertuning$iteraciones <- 300 #SUBIR, NO MENOS DE 200
PARAM$hyperparametertuning$xval_folds  <- 5
PARAM$hyperparametertuning$POS_ganancia  <- 78000
PARAM$hyperparametertuning$NEG_ganancia  <- -2000

PARAM$hyperparametertuning$semilla_azar  <- 100043  #Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy
