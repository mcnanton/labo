...
PARAM  <- list()
PARAM$experimento <- "FE9250_sh_ratio1"

PARAM$exp_input  <- "DR9141_sh"
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2 )
{
  ...
  predict_contrib <- predict(modelo,
                             newdata=    data.matrix(  dataset[ entrenamiento==TRUE, campos_buenos, with=FALSE]), #Idem dtrain
                             type = "contrib" 
  ...
  # Obtengo valores SHAP
  # Uso el equivalente a dtrain porque entiendo que es lo mismo que haciamos con feature_importance 
  # (pese a que no estaba explícito como argumento en la función)
  # shap_values  <- shap.values(xgb_model = modelo, 
  #                             X_train = data.matrix(  dataset[ entrenamiento==TRUE, campos_buenos, with=FALSE])) 
  # 
  # # Calculo el promedio en base a valores absolutos
  # dt_mean  <- shap_values$shap_score[, lapply(.SD,function(x) mean(abs(x)) ),]
  # dt_mean <- data.table( names(dt_mean), transpose( dt_mean ) )
  # colnames( dt_mean )  <-  c("columna","Shapley_mean" )
  # dt_mean2[ , Shapley_abs := abs( Shapley_mean ) ]
  # 
  # # Armo dataset ORDENADO  con variable | importancia
